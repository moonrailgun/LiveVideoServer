...
$rule_list = LVSRule::getRule(LVSRule::$gather_table_name);
...
if($method == 'del' && !empty($rule_id)){
  ...
  $result = LVSRule::delRule(LVSRule::$gather_table_name, $condition);

  if ($result >= 0) {
    SysLog::addLog(UserSession::getUserName(), 'DELETE', 'Rule', $rule_id, json_encode($rule_info));
    Common::exitWithSuccess('已删除规则','lvs/rule_gather.php' );
  }else{
    OSAdmin::alert("error");
  }
}
...
Template::display("lvs/rule_gather.tpl");
