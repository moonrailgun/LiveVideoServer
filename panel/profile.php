...
$user_name = $password = $real_name = $mobile = $email = $user_desc = $change_password = $show_quicknote = $old = $new = $new_repeat= '';
...
if (Common::isPost()) {
	if($change_password){
		if($new != $new_repeat){
			OSAdmin::alert("error",ErrorMessage::NOT_SAME_PWD);
		}else{
			...
			if($ret){
				if(strlen($new)<6){
					...
					Common::exitWithSuccess (ErrorMessage::PWD_UPDATE_SUCCESS,'/panel/logout.php');
				}
			}else{
